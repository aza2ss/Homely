(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=[];function t(t){e=t,window.addEventListener(`hashchange`,n),n()}function n(){let e=window.location.hash||`#/`,t=r(e);t&&t.handler?t.handler(t.params):window.location.hash=`#/`,a(e)}function r(t){let n=t.split(`?`)[0];for(let t of e){let e=t.path.split(`/`),r=n.split(`/`);if(e.length!==r.length)continue;let i=!0,a={};for(let t=0;t<e.length;t++)if(e[t].startsWith(`:`)){let n=e[t].slice(1);a[n]=r[t]}else if(e[t]!==r[t]){i=!1;break}if(i)return{handler:t.handler,params:a}}return null}function i(e){window.location.hash=e}function a(e){document.querySelectorAll(`nav a, .nav-link`).forEach(t=>{t.classList.remove(`active`);let n=t.getAttribute(`href`);(n===e||n===`#/catalog`&&e.startsWith(`#/catalog`))&&t.classList.add(`active`)})}var o=`homely_cart`,s=new class{constructor(){this.items=this.loadCart(),this.listeners=[]}loadCart(){try{let e=localStorage.getItem(o);return e?JSON.parse(e):[]}catch(e){return console.error(`Error loading cart from localStorage`,e),[]}}saveCart(){try{localStorage.setItem(o,JSON.stringify(this.items)),this.notifyListeners()}catch(e){console.error(`Error saving cart to localStorage`,e)}}addItem(e,t=1){let n=this.items.find(t=>t.product.id===e.id);n?n.qty+=t:this.items.push({product:e,qty:t}),this.saveCart()}removeItem(e){this.items=this.items.filter(t=>t.product.id!==e),this.saveCart()}updateQuantity(e,t){let n=this.items.find(t=>t.product.id===e);n&&(t<=0?this.removeItem(e):(n.qty=t,this.saveCart()))}getItems(){return[...this.items]}getTotal(){return this.items.reduce((e,t)=>e+t.product.price*t.qty,0)}getCount(){return this.items.reduce((e,t)=>e+t.qty,0)}clear(){this.items=[],this.saveCart()}subscribe(e){return this.listeners.push(e),e(),()=>{this.listeners=this.listeners.filter(t=>t!==e)}}notifyListeners(){this.listeners.forEach(e=>e()),window.dispatchEvent(new CustomEvent(`cart-updated`,{detail:{count:this.getCount()}}))}},c=(e,t)=>s.addItem(e,t),l=()=>s.getCount();function u(){return`
  ...
  `}function I(){let e=document.getElementById(`app`),t=()=>{e.innerHTML=F(),I()};document.querySelectorAll(`.cart-qty-minus`).forEach(e=>{e.addEventListener(`click`,e=>{let n=e.currentTarget.dataset.id,r=s.getItems().find(e=>e.product.id===n);r&&r.quantity>1&&(s.updateQuantity(n,r.quantity-1),t())})}),document.querySelectorAll(`.cart-qty-plus`).forEach(e=>{e.addEventListener(`click`,e=>{let n=e.currentTarget.dataset.id,r=s.getItems().find(e=>e.product.id===n);r&&(s.updateQuantity(n,r.quantity+1),t())})}),document.querySelectorAll(`.cart-item__remove`).forEach(e=>{e.addEventListener(`click`,e=>{let n=e.currentTarget.dataset.id;s.removeItem(n),t()})}),document.getElementById(`btn-checkout`)?.addEventListener(`click`,()=>{i(`#/checkout`)})}var L=`https://script.google.com/macros/s/AKfycbyZ8EpbSCWCYmf4nA6HRGr5dKlVjPcHYlhoP8xLslF0eDogDEHV1jOZVqiA6vqijC0/exec`;function R(){return`HML-${Math.floor(1e5+Math.random()*9e5)}`}async function z(e){let t=new Date().toLocaleString(`ru-RU`,{timeZone:`Asia/Almaty`}),n=(e.items||[]).map(e=>`${e.product.name} (x${e.quantity}) - ${e.product.price*e.quantity} ₸`).join(`; `),r={cash:`Наличными при получении`,card:`Банковская карта / Google Pay (Онлайн)`,installment:`Рассрочка / Kaspi Pay`}[e.paymentMethod]||e.paymentMethod||`Наличными`,i={date:t,orderNumber:e.orderNumber,customerName:e.name||``,phone:e.phone||``,email:e.email||``,city:e.city||``,address:e.address||``,comment:e.comment||``,paymentMethod:r,items:n,total:`${e.total} ₸`};V({...e,payload:i,createdAt:t});try{await fetch(L.trim(),{method:`POST`,mode:`no-cors`,headers:{"Content-Type":`application/json`},body:JSON.stringify(i)}),console.log(`Order sent to Google Sheet Webhook:`,i)}catch(e){console.error(`Error sending order to Google Sheets:`,e)}return{success:!0,orderNumber:e.orderNumber}}function B(){try{let e=localStorage.getItem(`homely_orders`);return e?JSON.parse(e):[]}catch(e){return console.error(`Error reading orders from localStorage`,e),[]}}function V(e){let t=B();t.push(e),localStorage.setItem(`homely_orders`,JSON.stringify(t))}function H(){let e=s.getItems();return e.length===0?`
      <div class="container page-section empty-cart" style="text-align: center; padding: 60px 0;">
      ...
      <div id="payment-modal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 1000; align-items: center; justify-content: center; padding: 20px;">
        <div style="background: #fff; width: 100%; max-width: 440px; border-radius: 16px; padding: 28px; box-shadow: 0 20px 40px rgba(0,0,0,0.2); animation: fadeInUp 0.3s ease-out;">
          ...
          <div style="background: #F7F8FA; border-radius: 8px; padding: 12px 16px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
            ...
            <strong id="modal-payment-amount" style="font-size: 18px; color: #1866AD;">${x(s.getTotal())}</strong>
          </div>
  ...
  `}function U(){let e=document.getElementById(`checkout-form`),t=document.getElementById(`checkout-phone`),n=document.getElementById(`payment-modal`),r=document.getElementById(`close-payment-modal`),a=document.getElementById(`pay-modal-form`),o=document.getElementById(`modal-payment-title`),c=document.getElementById(`pay-modal-submit`),l=null;t&&t.addEventListener(`input`,e=>{let t=e.target.value.replace(/\D/g,``);(t.startsWith(`7`)||t.startsWith(`8`))&&(t=`7`+t.slice(1)),t.length>0&&!t.startsWith(`7`)&&(t=`7`+t),e.target.value=t.length>0?`+`+t:``}),document.querySelectorAll(`input[name="payment"]`).forEach(e=>{e.addEventListener(`change`,()=>{document.querySelectorAll(`.payment-method-card`).forEach(e=>{e.style.borderColor=`#ddd`,e.style.background=`#fff`});let t=e.closest(`.payment-method-card`);t&&(t.style.borderColor=`#1866AD`,t.style.background=`#f0f7fc`)})}),r&&n&&r.addEventListener(`click`,()=>{n.style.display=`none`});let u=async e=>{let t=R();await z({...e,orderNumber:t}),s.clear(),i(`#/order-success/${t}`)};e?.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.querySelector(`input[name="payment"]:checked`)?.value||`cash`;l={name:document.getElementById(`checkout-name`).value,phone:document.getElementById(`checkout-phone`).value,email:document.getElementById(`checkout-email`).value,city:document.getElementById(`checkout-city`).value,address:document.getElementById(`checkout-address`).value,comment:document.getElementById(`checkout-comment`).value,paymentMethod:t,items:s.getItems(),total:s.getTotal()},t===`cash`?await u(l):(t===`installment`?(o&&(o.textContent=`Оформление рассрочки / Kaspi Pay`),c&&(c.innerHTML=`<span>Подтвердить рассрочку 0%</span>`)):(o&&(o.textContent=`Оплата картой / Google Pay`),c&&(c.innerHTML=`<span>Оплатить в 1 клик (Google Pay)</span>`)),n&&(n.style.display=`flex`))}),a?.addEventListener(`submit`,async e=>{e.preventDefault(),c&&(c.disabled=!0,c.innerHTML=`<span style="display:inline-block; animation: pulse 1s infinite;">Обработка платежа...</span>`),setTimeout(async()=>{n&&(n.style.display=`none`),l&&await u(l)},1200)})}function W(){return`
    <div class="container page-section">
  ...
  `}function G(){}function K(e){return`
  ...
  `}function q(){}var J=document.getElementById(`main-content`),Y=document.getElementById(`site-header`),X=document.getElementById(`site-footer`);Y.innerHTML=u(),X.innerHTML=f(),d();function Z(){window.scrollTo({top:0,behavior:`instant`})}async function Q(e,t,...n){J.innerHTML=`<div class="page-loading"><div class="spinner"></div></div>`,Z();try{J.innerHTML=await e(...n),t&&await t(...n),$()}catch(e){console.error(`Page render error:`,e),J.innerHTML=`
      <section class="page-section">
    ...
    `}}t([{path:`#/`,handler:()=>Q(E,D)},{path:`#/catalog`,handler:()=>Q(A,j)},{path:`#/catalog/:category`,handler:e=>Q(A,j,e.category)},{path:`#/product/:id`,handler:e=>Q(N,P,e.id)},{path:`#/cart`,handler:()=>Q(F,I)},{path:`#/checkout`,handler:()=>Q(H,U)},{path:`#/contacts`,handler:()=>Q(W,G)},{path:`#/order-success/:orderNumber`,handler:e=>Q(K,q,e.orderNumber)}]);function $(){let e=document.querySelectorAll(`.scroll-reveal:not(.visible)`);if(!e.length)return;let t=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&(e.target.classList.add(`visible`),t.unobserve(e.target))})},{threshold:.1,rootMargin:`0px 0px -50px 0px`});e.forEach(e=>t.observe(e))}window.addEventListener(`hashchange`,()=>{let e=window.location.hash||`#/`;document.querySelectorAll(`.nav-link`).forEach(t=>{let n=t.getAttribute(`href`);n===e||n===`#/catalog`&&e.startsWith(`#/catalog`)?t.classList.add(`active`):t.classList.remove(`active`)})}),console.log(`🏠 Homely — Интернет-магазин мебели загружен`);